...
| ----------------- | ---------------------- | ------------------------------- | ------------------------------- | ---------------------------------------------------------- |
| **Node.js** >= 18 | 运行 MCP Server        | [nodejs.org](https://nodejs.org) | [nodejs.org](https://nodejs.org) | [nodejs.org](https://nodejs.org)                          |
| **FFmpeg**        | 音视频提取、合并、拼接 | `brew install ffmpeg`           | `sudo apt install ffmpeg`       | [ffmpeg.org](https://ffmpeg.org/download.html) 加入 PATH |
| **Demucs**        | AI 人声分离            | `pip install demucs`            | `pip install demucs`            | `pip install demucs` ¹                                     |

> **Windows 用户注意：**
> - 请确保 `ffmpeg`、`ffprobe` 和 `demucs` 已加入系统 PATH，可在命令行直接调用
> - ¹ 如果 `pip` 命令不可用，请使用 `python -m pip install demucs` 代替
...
- 打开 YouTube https://www.youtube.com/account_advanced

  登录后，其中 `UC` 开头的就是频道 ID
...
- **跨平台**：不依赖 bash 或 curl，人声分离使用 Node.js spawn 链，下载使用原生 fetch，支持 macOS / Linux / Windows
